						...
						<div class="row">
							...
							<div class="col-12 form-group">
								...
								<input type="text" id="poster" value="${video != null ? video.poster: '' }" class="form-control">
									<input type="hidden" id="isEdit" value="${Edit}" class="form-control"/>
							</div>
						</div>
					</div>
					<div class="col-3">
...
//Gửi dữ liệu video khi nhấn nút Submit
$('#submitBtn').click(function () {
    $('#message').text(''); // Xóa thông báo cũ
...
if (isEdit = 'true') {
	alert('true');
url = '/movie-website/admin/video?action=edit&href='+ hrefOrigin;
} else {
	alert('false');
url = '/movie-website/admin/video?action=add';
}
    var formData = {
    ...
    if (!formData.title || !formData.href || !formData.poster) {
    ...
    $.ajax({
        ...
        window.location.href = "http://localhost:8080/movie-website/admin/video?action=view";
    }).fail(function (error) {
...
// Thay đổi ảnh xem trước khi nhập đường dẫn poster
$('#poster').change(function () {
    var newSrc = $('#poster').val();
    $('#imgPreview').attr('src', newSrc); // Đặt lại src cho thẻ img
});
